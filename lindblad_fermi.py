"""
File:           lindblad_fermi.py
Author:         Spenser Talkington (spenser@upenn.edu)
Description:    library for response functions of dissipative fermionic systems
...
class LindbladianFermiSP:
    ...
    def Sigma(self,k,G): #no -iG/2 out front--just block of the single particle lindbladian
        ...
        if self.nambu:
            return temp[2*n:4*n,0:2*n]
        else: #non-nambu
            return temp[2*n:3*n,0:n]
    ...
    def GR(self,k,G,w):
        ...
        if self.nambu:
            ...
            return np.linalg.inv(w*np.kron(s0,id)-self.Xi(k,G))
        else: #non-nambu
    ...
    def eigs(self,k,G): #non-nambu form
        n = self.N
        if self.nambu:
            d = 2
        else: #non-nambu
    ...
    def density(self,k,G):
        ...
        if self.nambu:
            ...
            return 0.5*(np.kron(s3,np.identity(self.N)) + term)
        else: #non-nambu
